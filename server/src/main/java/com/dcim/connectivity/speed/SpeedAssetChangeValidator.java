...
@Component
class SpeedAssetChangeValidator extends AbstractAssetChangeValidator<SpeedHistory> {
	...
	SpeedAssetChangeValidator(
		...
		super("SPEED", "speed", Set.of("speedName", "speedType"), history, SpeedHistory::getSpeedId, payloads);
	...
	protected void validateAddOrUpdate(
			AssetValidateCommand command, JsonNode body, SpeedHistory prior, List<ValidationIssue> issues) {
	...
	@Override
	protected void validateTerminate(SpeedHistory prior, ValidationContext context, List<ValidationIssue> issues) {
		List<Long> blocking = crossConnects.listCurrentBySpeedId(prior.getSpeedId()).stream()
